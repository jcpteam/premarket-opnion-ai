import { PrismaClient, MarketType, MarketStatus, OrderType, OrderSubType, OrderStatus } from '@prisma/client';
...
async function main() {
  ...
  // Create outcomes for multi-outcome market
  const teams = [
  ...
  // Resolved market for historical data
  ...
  const ethYesOutcome = await prisma.outcome.create({
    data: {
      ...
      currentPrice: 1.0, // Winning outcome
  ...
  const ethNoOutcome = await prisma.outcome.create({
    data: {
      ...
      currentPrice: 0.0, // Losing outcome
      totalShares: 45230.80,
      bestBid: 0.0,
      bestAsk: 0.0,
      spread: 0.0,
  ...
  // Active orders for binary market
  ...
  await prisma.order.create({
    data: {
      userId: trader1.id,
      marketId: multiMarket.id,
      outcomeId: teamOutcomes[0].id, // Brazil
      type: OrderType.BUY,
      orderType: OrderSubType.LIMIT,
      quantity: 200,
      price: 0.24,
  ...
  // First, let's create some orders that we can reference in trades
  ...
  await prisma.position.create({
    data: {
      userId: trader1.id,
      marketId: multiMarket.id,
      outcomeId: teamOutcomes[1].id, // Argentina
      quantity: 100,
      averagePrice: 0.21,
      totalCost: 21.00,
      currentValue: 22.00,
  ...
  // Create resolution for resolved market
